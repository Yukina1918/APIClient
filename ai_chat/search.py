...
SEARCH_PROVIDERS = (
    "tavily", "serper", "brave", "bing",
    "searxng", "duckduckgo", "custom",
)
NEEDS_KEY = {"tavily", "serper", "brave", "bing"}
NEEDS_ENDPOINT = {"searxng"}      # custom 自己在 handler 里检查 endpoint
...
def _bing(query, key, endpoint, count, timeout):
    ...
    return [SearchResult(r.get("name", ""), r.get("url", ""),
                         r.get("snippet", ""))
...
def _parse_ddg_html(page: str, count: int) -> list[SearchResult]:
    """从 HTML 页面里按 DuckDuckGo 的结构抽结果，用于 duckduckgo / custom。"""
...
def _duckduckgo(query, key, endpoint, count, timeout):
    ...
    return _parse_ddg_html(page, count)


# ---------------------------------------------------------------- Custom

def _custom(query, key, endpoint, count, timeout):
    """自定义搜索接口。

    规则：
      · endpoint 必填；可含 {query} 占位符，会自动替换为 URL 编码的问题；
        不含占位符时，自动在末尾追加 ?q=<query>&num=<count>
      · key 可选；填了就带 Authorization: Bearer <key>
      · 响应优先按 JSON 解析，自动找 results / items / data / hits / organic
        数组，字段兼容 title|name、url|link|href、content|snippet|description
      · JSON 解析失败 → 按 DuckDuckGo 的 HTML 结构兜底
    """
    if not endpoint:
        raise SearchError("CUSTOM_ENDPOINT_MISSING")

    if "{query}" in endpoint:
        url = endpoint.replace("{query}", urlparse.quote_plus(query))
    else:
        sep = "&" if "?" in endpoint else "?"
        url = endpoint + sep + urlparse.urlencode({"q": query, "num": count})
    ...
    if isinstance(arr, list):
        for item in arr[:count]:
            ...
            results.append(SearchResult(
                ...
                str(item.get("url") or item.get("link")
                    or item.get("href") or ""),
                str(item.get("content") or item.get("snippet")
    ...
    # 2) 退化为 HTML
    return _parse_ddg_html(raw, count)
...
def run_search(search_cfg: dict, query: str,
    ...
    if provider in NEEDS_ENDPOINT and not endpoint:
        raise SearchError("SEARXNG_ENDPOINT_MISSING")
...
def build_search_prompt(question: str, results: list[SearchResult]) -> str:
    ...
    for idx, item in enumerate(results, 1):
        ...
        if len(snippet) > 600:
            snippet = snippet[:600] + "…"
        blocks.append("[%d] %s\n%s\n%s" % (
            idx, item.title or "(无标题)", item.url, snippet))
...
def format_sources(results: list[SearchResult], limit: int = 5) -> str:
    ...
    for idx, item in enumerate(results[:limit], 1):
        lines.append("[%d] %s  %s" % (
            idx, item.title[:60] or "(无标题)", item.url))
    return "\n".join(lines)