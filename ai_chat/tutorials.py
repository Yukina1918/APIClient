...
1. Open Settings from the toolbar or File → Settings.
2. On the "API & Model" tab pick a provider preset —
   the endpoint and a typical model are filled in for you.
3. Paste the API key (tick "Show" to double-check it).
4. Adjust temperature (0–2) and timeout if needed.
5. Click "Test connection"; a success dialog means you are good.
6. Click "Save".
...
If it speaks the OpenAI /chat/completions protocol, choose "Custom"
and type the URL, key and model name yourself.
...
3. 本软件：设置 → 联网搜索 → 服务选 Tavily → 粘贴 Key。
4. 点「保存」，回主界面勾选「联网搜索」。
...
勾选工具栏的「联网搜索」后，每次提问都会先联网检索。
不勾选则只走模型自身知识。

回答下方会列出本次引用的来源编号，方便你核对原文。
...
Settings → Web Search → pick DuckDuckGo. No key required.
...
3. Settings → Web Search → provider Tavily → paste the key.
...
With "Web search" ticked, every question triggers a search first.
Cited sources are listed under each answer.""",
    },
...
Q：提示「请求超时」？
A：网络不通，或需要走代理。在设置的「代理」里填写。

Q：一直不出字？
A：部分推理模型首字很慢，把超时时间调大到 300 秒试试。

Q：回答里有很多 ** 星号？
A：设置 → 对话与输出 → 把「Markdown 精简」调成「标准」或「强力」。
...
Q: "Request timed out"
A: Network issue or you need a proxy. Fill in the Proxy field.
...
A: Settings → Chat & Output → set "Markdown cleanup" to Normal or Strong.

Q: How do I turn web search off completely?
A: Just untick "Web search" in the toolbar.

Q: Where is my data stored?
A: All under data/ next to the program:
   · data/config.json    UI and model preferences
   · data/keys.json      your API keys
   · data/logs/*.txt     chat logs
...
A: Use the "📋 History" dropdown on the top-right, or
   Session → New session.

Q: How many context messages are sent?
A: Settings → Chat & Output → "Context messages", default 20.
   Only the most recent N messages are sent per request.""",
    },
}